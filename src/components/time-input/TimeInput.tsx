    ...
    <div className={fieldClasses}>
      ...
      <div className={styles.control}>
        <input
          ...
          id={inputId}
          type="time"
          className={[styles.input, error ? styles.invalid : undefined, className].filter(Boolean).join(' ')}
          aria-invalid={error ? true : undefined}
          aria-describedby={descriptionIds || undefined}
        />
        <Clock3 className={styles.icon} aria-hidden="true" />
      </div>
      {hasMessage ? <span id={messageId} className={error ? styles.error : styles.hint}>{error ?? hint}</span> : null}
    </div>
  )
}
